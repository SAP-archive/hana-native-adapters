...
public class SQLRewriter {
	...
	public static String rewriteSQL(String sql)  throws AdapterException 
	{
		...
		try
			{								
				BQAdapter.logger.debug("rewrite in query: " +sql);
				ExpressionBase query = ExpressionParserUtil.buildQuery(sql, messageList);
				BQAdapter.logger.debug("rewrite final query: " +query);
				
				if (query != null)
				{
					String sqlRewrite = regenerateSQL(query);
					BQAdapter.logger.trace(sql);
					BQAdapter.logger.trace(sqlRewrite);
					return sqlRewrite;
				}
				else
				{
					...
					{
						BQAdapter.logger.error(e.getText());					
					}
					throw new AdapterException("Parse failed. See earlier logs");
				}
			}
			catch ( Exception e ) {
				BQAdapter.logger.error("SQL Rewrite failed.", e);
	...
	private static String regenerateSQL(Query query) throws Exception
	{
		...
		BQAdapter.logger.debug("### regenerateSQL Query : "+sql.toString());
	...
	private static String printExpression(ExpressionBase val) throws Exception
	{
		...
		StringBuffer str = new StringBuffer();
	//	BQAdapter.logger.info("### printExpression : "+val.getType());
		switch(val.getType())
		{
		...
		case TABLE:
		{
			...
			//Bigquery table syntax - project.schema.table
			String tablePrefix = "`"+BQAdapter.projectID+"`."+BQAdapter.dataSetID+".";
			String uniquename = tablePrefix+tab.getUnquotedName();
		...
		{
			Expression exp = (Expression)val;
			//truncate microseconds to 6 digit precision
			String ts = exp.getValue();   //'2019-05-21 19:05:43.000000'
			BQAdapter.logger.debug("### ts : "+ts);
			String[] parts = ts.split("\\.");  //need to escape the . or use [.] to split on a .
			String ms = parts[1].substring(0,6)+"'";
			str.append("TIMESTAMP("+parts[0]+"."+ms+")");  //old: exp.getValue()
		...
		//BQAdapter.logger.info("### printExp : "+str.toString());
		return str.toString();
	}
